...
function extract(source_path) {
	...
	try {
		...
		_.each(files, function(file_name) {
			...
			if (found.length > 0) {
				logger.debug(file_path + ': ' + found.length + ' strings');
		...
		logger.info("Found " + strings.length + " unique i18n strings in source");
...
function readLanguages(i18n_path) {
	var languages = [],
		files = fs.readdirSync(i18n_path);
	...
	_.each(files, function(file) {
		...
		if (stat.isDirectory()) {
			...
			if (!fs.existsSync(path.join(i18n_path, file, 'strings.xml'))) {
				logger.warn('Language \'' + file + '\' has no strings.xml - we\'ll create it for you when you apply.');
			}
...
function readStrings(file_path) {
	var doc;

	if (doc = xml.fromFile(file_path)) {
		...
		logger.warn('Could not read strings from: ' + file_path);
		return [];
	}
}
...
exports.run = function(env) {
	...
	var languages = config.language ? [config.language] : readLanguages(path.join(config.project.path, 'i18n'));
	...
	_.each(languages, function(language) {
		...
		var strings_existing = readStrings(language_path);
	...
	logger.info('Found ' + strings_new_count + ' strings missing in one or more string.xml files.');
	...
	if (!config.apply) {
		logger.info('Add "-a" or "--apply" to append the new strings to the strings.xml file(s).');
	}
};